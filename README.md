...
### 8-3. 기술적 의사결정 및 구현 방식
결국 MSVC 호환성을 위해 코드를 수정하는 대신, **기존의 VLA 방식을 유지**하는 것으로 결정하였으며 그 이유는 다음과 같음.

1. **런타임 퍼포먼스 고려:** MSVC 호환성을 맞추기 위해 렌더링 함수(`draw_game`) 내부에서 매 프레임마다 동적 할당(`malloc`)과 해제(`free`)를 반복할 경우, 힙(Heap) 메모리 접근 오버헤드로 인한 성능 저하 및 메모리 파편화가능성이 존재함.
