...
## 4. 개발 중 발생한 핵심 문제 & 해결 과정

### 4.1 맵 데이터 define으로 고정 배열 → 동적 메모리 구조로 개선
...
### 4.2 화면 깜빡임(Flickering)과 커서 잔상 문제
...
### 4.3 점프 관련 로직 수정 + 방향 점프(관성) 이동 시 충돌 버그
...
### 4.4 OS별 입력 버퍼 & 인코딩 문제

#### 4.4.1 Linux / macOS 입력 지연
...
## 6. 추가 기능 - 효과음 시스템 (Beep) 및 크로스플랫폼 처리
