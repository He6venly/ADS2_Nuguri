...
## 8. 개발 환경 차이에 따른 이식성 분석 및 해결하지 못한 문제..

### 8-1. 문제 상황 (Issue)
* **현상:** Linux(GCC) 및 macOS(Clang) 환경에서는 정상적으로 컴파일 및 실행되는 코드가 Windows(MSVC) cl로 컴파일하는 환경에서는 `C2057: 상수 식이 필요합니다`, `C2466: 상수 크기 0의 배열을 할당할 수 없습니다` 등의 컴파일 에러를 발생시킴.
* **원인:** `draw_game` 및 `load_maps` 함수 내에서 맵의 가로/세로 크기 변수(`MAP_WIDTH`, `MAP_HEIGHT`)를 사용하여 가변길이 배열(`char array[MAP_HEIGHT][MAP_WIDTH]`)을 선언한 부분에서 발생함.

### 8-2. 원인 분석 (Technical Analysis)
* **가변 길이 배열 (VLA, Variable Length Array):** 해당 문법은 C99 표준에 도입된 기능으로, 컴파일 시점이 아닌 런타임(실행) 시점에 배열의 크기를 결정하여 스택(Stack) 메모리에 할당하는 방식임.
* **컴파일러별 차이:**
    * **GCC / Clang:** C99 표준을 준수하므로 VLA를 기본적으로 지원하여 문제없이 컴파일됨.
    * **MSVC (Visual Studio):** Microsoft 컴파일러는 스택 오버플로우와 같은 보안 취약점 우려 및 C11 표준에서의 '선택적 지원(Optional)' 사항임을 근거로 선언 시 가변길이 배열을 공식적으로 지원하지 않음.

### 8-3. 기술적 의사결정 및 구현 방식 (Implementation Rationale)
결국 MSVC 호환성을 위해 코드를 수정하는 대신, **기존의 VLA 방식을 유지**하는 것으로 결정하였으며 그 이유는 다음과 같음.

1. **런타임 퍼포먼스 고려:** MSVC 호환성을 맞추기 위해 렌더링 함수(`draw_game`) 내부에서 매 프레임마다 동적 할당(`malloc`)과 해제(`free`)를 반복할 경우, 힙(Heap) 메모리 접근 오버헤드로 인한 성능 저하 및 메모리 파편화(Fragmentation) 가능성이 존재함.
2. **코드 가독성 및 직관성:** 맵의 크기에 맞는 2차원 배열을 직관적으로 선언하는 방식이 동적 할당 및 이중 포인터를 사용하는 방식보다 코드의 복잡도를 낮추고 로직을 명확하게 표현함.
3. 개발 환경을 Visual Stuido Code(vsc)와 팀원들은 Mac환경에서 진행하다 보니 모든 컴파일러가 (C99 기준이라 에러로 잡히지도 않아 놓쳤던 부분이였음.)
4. 결론적으로 현재 짠 코드구조를 모조리 갈아엎고 동적 할당 로직을 새로 구현해야 하는데, 마감시간이 임박했을때 이 문제를 인지하여 해결하지 못하고 보고서에 리포트하는 식으로라도 알리고 싶었습니다.
---
